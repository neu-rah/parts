...
namespace Parts {
  ...
  namespace PartsDef {
    //Pair class
    template<typename Cfg,typename Fst,typename Snd>
    struct Pair {
      Fst _head;
      Snd _tail;
      using Head=Fst;
    ...
    struct StaticList:Fst {
      ...
      auto map(Args... args) 
        ->Pair<Cfg,decltype(API().operator()(head(),args...)),decltype(tail().template map<API,Args...>(args...))>
      {
        return Pair<Cfg,decltype(API().operator()(head(),args...)),decltype(tail().template map<API,Args...>(args...))>(
    ...
    //list end
    template<typename Cfg,typename Fst>
    struct StaticList<Cfg,Fst,Nil>:Fst {
      using Idx=typename Cfg::Idx;
      using Fst::Fst;
      using This=StaticList<Cfg,Fst,Nil>;
      inline StaticList() {}
